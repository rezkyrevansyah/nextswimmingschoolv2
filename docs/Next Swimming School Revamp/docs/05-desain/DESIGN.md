...
`kind` yang sah: `active`, `present`, `paid`, `approved`, `draft`, `published`, `paid_off`, `written_off`, `cancelled`, `pending`, `unpaid`, `partial`, `suspend`, `suspended`, `archived`, `inactive`, `rejected`, `absent`, `holiday`, `substitute`, `manual`, `excused`, `sick`, `late`, `telat`, `free`, `school_covered`.

Dua kind slip gaji memakai tangga yang sudah ada, bukan warna baru: `draft` memakai tangga `archive`, `published` memakai tangga `ok`. Keduanya ditambahkan karena alur slip butuh keduanya dan sebelumnya tidak ada padanannya, sehingga kode berisiko mengarang lencana sendiri.

Tiga status kasbon ikut masuk dengan alasan yang sama: `paid_off` memakai tangga `ok`, `written_off` memakai `danger`, `cancelled` memakai `archive`. Status `active` sudah ada sejak awal. Tagihan siswa juga butuh `partial` (tangga `warn`) karena saringan Income memakai lima nilai: `unpaid`, `paid`, `partial`, `school_covered`, `free`.
