...
**Yang dapat dilakukan:** menyunting nama panggilan, gender, tanggal lahir, telepon, spesialisasi, bio, alamat, pendidikan; rekening (wajib untuk membuka kunci); kata sandi; avatar; sertifikat (awal `pending` → Admin **Approvals**); menampilkan `public_id` (kode `CO`) plus QR permanen; keluar.
