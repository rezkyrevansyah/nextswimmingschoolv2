...
Pelatih / siswa / staf (tanggal nanti)
   ...
   pengajuan pending
        │
        ├── Admin atau Owner setuju ──► approved ──► efek absensi (+ delegasi jika pelatih)
        ├── Admin atau Owner tolak  ──► rejected  (+ alasan; tidak ada efek absensi)
        └── pemohon batal ──► cancelled (hanya selama pending)
```

**Dua pintu, satu penutup.** Admin menyetujui dari pusatnya sendiri di **Leave Requests**. Owner menyetujui lintas pusat dari menu **Leave Requests** miliknya, sebagai jaring pengaman kalau Admin pusat itu belum sempat menindak. Keduanya menulis ke baris `pending` yang sama; begitu satu pihak menyetujui atau menolak, baris itu tertutup untuk yang lain. Aksi kedua pada baris yang sudah tertutup harus ditolak dengan pesan bahwa baris itu sudah diputuskan (oleh peran dan pusat mana), bukan diproses ulang secara diam-diam. Log aktivitas mencatat peran aktor supaya jelas keputusan datang dari Admin pusat terkait atau dari Owner.
...
5. Kirim → `pending`.
6. Admin pusat kelas, atau Owner lewat menu lintas pusatnya: setuju / ganti pengganti lalu setuju / tolak plus alasan. Siapa pun yang lebih dulu menutup pengajuan ini.
...
4. Admin pusat siswa, atau Owner lewat menu lintas pusatnya, setuju atau tolak plus alasan.
...
**Tanggal nanti.** Pengajuan `pending` ke Admin **Leave Requests** sub-tab Staff, atau ke Owner lewat menu lintas pusatnya. Tanpa delegasi. Setelah `approved`, clock-in di tanggal itu ditolak. Setelah `rejected`, staf masih bisa clock-in.
...
|---|---|---|---|
| Delegasi + Sick/Izin pelatih | Coach overlay izin | Admin **Leave Requests** → Coach, atau Owner **Leave Requests** lintas pusat | Class/Home pengganti; Attendance; Class Activity |
| Sick/Izin siswa | Student **Leave** | Admin **Leave Requests** → Student, atau Owner **Leave Requests** lintas pusat | Roster pelatih; Student Attendance; School Excel |
| Sick/Izin staf hari ini | Staff Home | Tidak menunggu; Bell Admin | Attendance Staff |
| Sick/Izin staf tanggal nanti | Staff (pengajuan) | Admin **Leave Requests** → Staff, atau Owner **Leave Requests** lintas pusat | Clock-in dikunci di tanggal itu |
