    ...
    <Card padded={false}>
      ...
      <div className="px-4 sm:px-5 pt-4 pb-3 border-b border-line space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <SectionTitle sub={`${students.length} ${"Student".toLowerCase()}`}>{"Student Swimming Report Cards"}</SectionTitle>
          ...
          {!selectMode ? (
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button type="button" onClick={() => setSelected(new Set())}
                className="text-xs font-semibold text-ink-mute hover:underline">{"Cancel"}</button>
              <Btn variant="primary" size="sm" icon="download" disabled={selected.size === 0 || bulkDownloading} onClick={handlePrintSelected}>
                {bulkDownloading ? bulkDownloadingLabel : `${"PDF"} (${selected.size})`}
              </Btn>
              <Btn variant="ghost" size="sm" onClick={() => { setSelectMode(false); setSelected(new Set()); }}>{"Close"}</Btn>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            <option value="status:desc">{"Complete"}</option>
            <option value="status:asc">{"Incomplete"}</option>
          </select>
        ...
        {showFilters && (
          <div className="bg-paper-tint border border-line rounded-xl p-4 grid sm:grid-cols-3 gap-3">
            ...
            <div>
              <div className="text-[10px] uppercase tracking-widest font-bold text-ink-faint mb-1.5">{"Status"}</div>
      ...
      {loading ? (
        <div className="p-10 text-center text-sm text-ink-mute">{"Loading partner school data…"}</div>
      ) : (
        <>
          {/* Desktop table */}
              ...
              <thead>
                <tr className="text-[11px] uppercase tracking-widest text-ink-faint font-bold border-b border-line">
                  ...
                  <th className="text-left py-3 font-bold cursor-pointer select-none group" onClick={() => toggleSort("class")}>
                    {"Class"} <SortIcon col="class" sortBy={sortBy} sortDir={sortDir} />
                  </th>
                  ...
                  <th className="text-right py-3 px-5 font-bold">{"Action"}</th>
                </tr>
              </thead>
                ...
                {paginated.map(s => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="text-ink-soft text-sm"><NoTranslate>{s.school_grade ?? "—"}</NoTranslate></td>
                      ...
                      <td>{s.is_filled ? <Status kind="approved">{"Complete"}</Status> : <Status kind="pending">{"Incomplete"}</Status>}</td>
                ...
                {filteredSorted.length === 0 && (
                  <tr>
                    <td colSpan={selectMode ? 7 : 6} className="py-14 text-center">
                      ...
                      <div className="text-sm font-semibold text-ink-mute">{"No student report cards found matching the criteria."}</div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-ink truncate"><NoTranslate>{s.full_name}</NoTranslate></div>
                  ...
                  <div className="flex flex-col items-end gap-2 shrink-0">
                    {s.is_filled ? <Status kind="approved">{"Complete"}</Status> : <Status kind="pending">{"Incomplete"}</Status>}
          ...
          {totalPages > 1 && (
            <div className="px-4 sm:px-5 py-3.5 border-t border-line flex items-center justify-between flex-wrap gap-3">
              <span className="text-xs text-ink-mute tabular-nums">
                {filteredSorted.length} {"Student".toLowerCase()} · {`Page ${safePage + 1} of ${totalPages}`}
              </span>
              <div className="flex items-center gap-1">
                ...
                <button type="button" disabled={safePage === 0} onClick={() => setPage(p => p - 1)}
                  className="px-3 py-1.5 rounded-lg border border-line text-ink-mute text-sm disabled:opacity-40 disabled:cursor-not-allowed hover:bg-paper-tint transition hidden sm:inline-flex">‹ {"Previous"}</button>
                ...
                <button type="button" disabled={safePage === totalPages - 1} onClick={() => setPage(p => p + 1)}
                  className="px-3 py-1.5 rounded-lg border border-line text-ink-mute text-sm disabled:opacity-40 disabled:cursor-not-allowed hover:bg-paper-tint transition hidden sm:inline-flex">{"Next"} ›</button>
