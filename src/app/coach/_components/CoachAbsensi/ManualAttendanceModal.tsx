...
export default function ManualAttendanceModal({ hook, classes }: { hook: ReturnType<typeof useCoachAbsensi>; classes: ClassRow[] }) {
  ...
  return (
    <Modal open={openManual} onClose={closeManualModal} title={t("coach.absen.manualModalTitle")}
      footer={<><Btn variant="ghost" onClick={closeManualModal}>{t("common.actions.cancel")}</Btn><Btn variant="primary" onClick={saveManualAtt} disabled={saving}>{saving ? t("coach.absen.savingBtn") : t("coach.clockIn.submitBtn")}</Btn></>}>
