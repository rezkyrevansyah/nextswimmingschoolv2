...
import { STUDENT_DB_STATUSES, uiToStudentDb } from "@/lib/attendance";
...
export default function ManualAttendanceModal({ hook, classes }: { hook: ReturnType<typeof useCoachAbsensi>; classes: ClassRow[] }) {
  const {
    ...
    manualDate, setManualDate, manualSessionDates, studentAtt, attStatus, setAttStatus,
      ...
      <div className="space-y-4">
        ...
        {studentAtt.length > 0 && (
          <div className="space-y-2">
            {studentAtt.map((m) => (
              <div key={m.student_id} className="flex items-center gap-3 p-3 rounded-xl border border-line">
                <Avatar name={m.student?.full_name ?? "?"} src={m.student?.avatar_url ?? undefined} size={36} />
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-ink text-sm truncate"><NoTranslate>{m.student?.full_name}</NoTranslate></div>
                ...
                <div className="flex flex-wrap gap-1">
                  {STUDENT_DB_STATUSES.map(id => {
                    const label = id === "hadir" ? "Present" : id === "telat" ? "Late" : id === "izin" ? "Leave" : id === "sakit" ? "Sick" : "Absent";
                    const active = (attStatus[m.student_id] ?? uiToStudentDb("present")) === id;
                    ...
                    return (
                      <button key={id} onClick={() => setAttStatus(s => ({ ...s, [m.student_id]: id }))}
