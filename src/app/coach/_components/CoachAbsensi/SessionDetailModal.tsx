...
export default function SessionDetailModal({ hook }: { hook: ReturnType<typeof useCoachAbsensi> }) {
  ...
  return (
    <Modal
      ...
      title={detailSesi ? <><NoTranslate>{detailSesi.className}</NoTranslate> — {fmtDate(detailSesi.date)}</> : ""}
      size="sm"
      footer={<Btn variant="ghost" onClick={() => setDetailSesi(null)}>{"Close"}</Btn>}
    >
      {loadingDetail ? (
        ...
        <div>
          ...
          <div className="space-y-0.5">
            {detailRows.map(r => {
              ...
              <div key={r.member_id} className="flex items-center gap-3 py-2.5 border-b border-line last:border-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-semibold text-ink truncate"><NoTranslate>{r.full_name}</NoTranslate></div>
                </div>
                <Status kind={kind}>
                  {label}
                </Status>
            ...
            {detailRows.length === 0 && (
              <div className="py-4 text-center text-ink-mute text-sm">{"No attendance data for this session."}</div>
            )}
          </div>
        </div>
      )}
