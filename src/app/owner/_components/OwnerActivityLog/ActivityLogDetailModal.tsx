...
export default function ActivityLogDetailModal({ hook }: { hook: ActivityLogDataHook }) {
  ...
  return (
    <Modal
      ...
      size="md"
      footer={<Btn variant="ghost" onClick={() => setDetailLog(null)}>{"Close"}</Btn>}
    >
      {detailLog && (
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="bg-paper-tint/50 rounded-xl p-3 border border-line/60">
              <div className="text-[10px] text-ink-faint uppercase tracking-wider font-bold mb-1">{"Time"}</div>
              <div className="font-semibold text-ink">{fmtShortDate(detailLog.created_at)} {fmtTime(detailLog.created_at)}</div>
            </div>
            <div className="bg-paper-tint/50 rounded-xl p-3 border border-line/60">
              <div className="text-[10px] text-ink-faint uppercase tracking-wider font-bold mb-1">{"Center"}</div>
              <div className="font-semibold text-ink"><NoTranslate>{branchName(detailLog)}</NoTranslate></div>
            </div>
            <div className="bg-paper-tint/50 rounded-xl p-3 border border-line/60">
              <div className="text-[10px] text-ink-faint uppercase tracking-wider font-bold mb-1">{"By"}</div>
            ...
            <div className="bg-paper-tint/50 rounded-xl p-3 border border-line/60">
              <div className="text-[10px] text-ink-faint uppercase tracking-wider font-bold mb-1">{"Entity"}</div>
            ...
            <div className="bg-paper-tint/50 rounded-xl p-3 border border-line/60">
              <div className="text-[10px] text-ink-faint uppercase tracking-wider font-bold mb-1">{"Action"}</div>
            ...
            {detailLog.entity_label && (
              <div className="bg-paper-tint/50 rounded-xl p-3 border border-line/60">
                <div className="text-[10px] text-ink-faint uppercase tracking-wider font-bold mb-1">{"Subject"}</div>
          ...
          <div className="border-t border-line pt-3">
            <div className="text-[10px] text-ink-faint uppercase tracking-wider font-bold mb-1.5">{"Description"}</div>
          ...
          {detailLog.meta && Object.keys(detailLog.meta).length > 0 && (
            <div>
              <div className="text-[10px] text-ink-faint uppercase tracking-wider font-bold mb-1.5">{"Detail"}</div>
