...
export default function FaqTab() {
  ...
  const del = async (item: FaqItem) => {
    const yes = await confirm({ title: "Delete this FAQ?", body: item.question || "This question will be removed from the landing page.", danger: true });
    if (!yes) return;
  ...
  return (
    <Card>
      <div className="flex items-center justify-between">
        <SectionTitle sub={"Questions and answers shown in the FAQ section"}>{"FAQ"}</SectionTitle>
      ...
      <div className="mt-4 space-y-2">
        {items.map((item) => (
          <div key={item.id} className="rounded-xl bg-paper-tint border border-line p-3 flex items-start gap-2">
            <div className="flex-1 min-w-0">
              <div className="text-sm font-bold text-ink">{item.question ? <NoTranslate>{item.question}</NoTranslate> : "Unnamed"}</div>
        ...
        {items.length === 0 && <div className="py-8 text-center text-ink-mute text-sm">{"No FAQ items yet."}</div>}
      </div>

      <Modal open={showModal} onClose={() => setShowModal(false)} title={editItem ? "Edit FAQ" : "Add FAQ"} size="sm"
        footer={<><Btn variant="ghost" onClick={() => setShowModal(false)}>{"Cancel"}</Btn><Btn variant="primary" onClick={save} disabled={saving || !form.question.trim() || !form.answer.trim()}>{saving ? "Saving…" : "Save"}</Btn></>}>
        <div className="space-y-3">
          <Field label={"Question"}><Input value={form.question} onChange={(e) => setForm({ ...form, question: e.target.value })} placeholder={"What age can start swimming lessons?"} /></Field>
          <Field label={"Answer"}><Textarea rows={4} value={form.answer} onChange={(e) => setForm({ ...form, answer: e.target.value })} placeholder={"We accept students starting from 4 years old, adjusted to each child's readiness."} /></Field>
