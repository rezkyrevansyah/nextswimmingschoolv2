    ...
    <section className="bg-paper-tint">
      <div className="max-w-7xl mx-auto px-4 lg:px-8 py-20 lg:py-28">
        ...
        <div className="grid md:grid-cols-3 gap-6 mt-12 items-center">
          {/* Card biasa kiri */}
          {first && (
            <Card className="transition-all duration-300 hover:shadow-lift hover:-translate-y-1">
              <div className="flex gap-0.5">
                {Array.from({ length: 5 }).map((_, k) => (
                  <Icon key={k} name="star" className="w-4 h-4 text-amber-400" strokeWidth={0} />
                ))}
              </div>
              <p className="mt-4 text-[15px] leading-relaxed text-ink-soft min-h-[72px]">&ldquo;{first.text}&rdquo;</p>
          ...
          {featured && (
            <div className="rounded-2xl border border-ocean-700 bg-ocean-700 p-6 lg:scale-[1.04] shadow-lift transition-all duration-300 hover:shadow-xl hover:-translate-y-1.5 text-white">
              <div className="flex gap-0.5">
                {Array.from({ length: 5 }).map((_, k) => (
                  <Icon key={k} name="star" className="w-4 h-4 text-wave-200" strokeWidth={0} />
                ))}
              </div>
              <p className="mt-4 text-[15px] leading-relaxed text-white/90 min-h-[72px]">&ldquo;{featured.text}&rdquo;</p>
          ...
          {last && (
            <Card className="transition-all duration-300 hover:shadow-lift hover:-translate-y-1">
              ...
              <p className="mt-4 text-[15px] leading-relaxed text-ink-soft min-h-[72px]">&ldquo;{last.text}&rdquo;</p>
              <div className="mt-5 pt-5 border-t border-line/40 flex items-center gap-3">
