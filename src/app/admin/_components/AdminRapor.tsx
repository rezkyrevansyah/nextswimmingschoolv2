...
function AdminCoachReviews({ branchId }: { branchId: string }) {
  ...
  if (loading) return <div className="text-ink-mute text-sm py-4">{"Loading reviews…"}</div>;
  if (reviews.length === 0) return (
    <div className="text-center py-8 text-ink-mute text-sm bg-paper-tint rounded-2xl">{"No reviews from students yet."}</div>
  );
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {coachAvg.map(c => (
            <button key={c.id} onClick={() => setFilterCoach(filterCoach === c.id ? "all" : c.id)}
              ...
              <div className={`font-semibold text-sm truncate ${filterCoach === c.id ? "text-white" : "text-ink"}`}><NoTranslate>{c.name}</NoTranslate></div>
      ...
      <div className="space-y-2.5">
        ...
        {filtered.length === 0 && <p className="text-ink-mute text-sm text-center py-4">{"No reviews for this coach."}</p>}
      </div>
    </div>
  );
}
...
export default function AdminRapor({ branchId }: { branchId: string }) {
  ...
  const create = async () => {
    if (!form.label || !form.date_from || !form.date_to) return toast.error("All fields are required");
    setSaving(true);
    ...
    setSaving(false);
    if (error) return toast.error("Failed to create period", error.message);
    toast.success("Report period opened");
    setOpenAdd(false);
    load();
  };

  const closePeriod = async (id: string) => {
    const ok = await confirm({ title: "Close period?", body: "Coaches will no longer be able to fill in report cards after the period is closed.", confirmLabel: "Close", danger: true });
    if (!ok) return;
  ...
  const saveEdit = async () => {
    ...
    if (!editForm.label || !editForm.date_from || !editForm.date_to) return toast.error("All fields are required");
    setSavingEdit(true);
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div><h2 className="font-display font-bold text-2xl">{"Report Period"}</h2><p className="text-ink-mute text-sm mt-0.5">{"Open & manage report card entry periods."}</p></div>
        <Btn variant="primary" icon="plus" onClick={() => { setForm({ label: "", date_from: "", date_to: "" }); setOpenAdd(true); }}>{"Open New Period"}</Btn>
      </div>
      {activePeriod && (
        <div className="bg-ocean-700 text-white rounded-2xl border border-ocean-700 shadow-card p-5 relative overflow-hidden">
          ...
          <div className="relative">
            <div className="flex items-center gap-2 text-wave-200 text-xs font-bold uppercase tracking-widest"><span className="w-2 h-2 rounded-full bg-wave-300 animate-pulse" /> {"Active period"}</div>
            <div className="mt-2 font-display font-extrabold text-3xl"><NoTranslate>{activePeriod.label}</NoTranslate></div>
            <div className="text-white/80 mt-1">{fmtDate(activePeriod.date_from)} – {fmtDate(activePeriod.date_to)}</div>
            <div className="mt-4 flex items-center gap-2">
              <button onClick={() => openEditModal(activePeriod)} className="px-3 py-1.5 rounded-lg text-sm font-semibold bg-white/15 hover:bg-white/25 text-white border border-white/30 transition-colors">{"Edit period"}</button>
              <button onClick={() => closePeriod(activePeriod.id)} className="px-3 py-1.5 rounded-lg text-sm font-semibold bg-white/10 hover:bg-white/20 text-white/80 hover:text-white border border-white/20 transition-colors">{"Close period"}</button>
            </div>
          </div>
        </div>
      )}
      {!loading && periods.length === 0 && <p className="text-ink-mute">{"No report periods yet."}</p>}
      {periods.filter(p => !p.is_open).length > 0 && (
        <Card>
          <SectionTitle sub={"Period history"}>{"Past Periods"}</SectionTitle>
          <div className="space-y-2">
            {periods.filter(p => !p.is_open).map(p => (
              <div key={p.id} className="flex items-center gap-3 p-3 rounded-xl bg-paper-tint">
                <div className="flex-1"><div className="font-semibold text-ink"><NoTranslate>{p.label}</NoTranslate></div><div className="text-xs text-ink-mute">{fmtDate(p.date_from)} – {fmtDate(p.date_to)}</div></div>
                <Status kind="archived">{"Closed"}</Status>
                ...
                <button onClick={() => reopenPeriod(p.id)} className="p-1.5 rounded hover:bg-ok-50 text-ink-mute hover:text-ok-600" title={"Reopen"}><Icon name="check" className="w-4 h-4" /></button>
              </div>
            ))}
          </div>
        </Card>
      ...
      <div>
        <SectionTitle sub={"Student reviews of coaches"} action={null}>{"Coach Reviews"}</SectionTitle>
        <AdminCoachReviews branchId={branchId} />
      </div>

      <Modal open={openAdd} onClose={() => setOpenAdd(false)} title={"Open New Report Period"} size="sm"
        footer={<><Btn variant="ghost" onClick={() => setOpenAdd(false)}>{"Cancel"}</Btn><Btn variant="primary" onClick={create} disabled={saving}>{saving ? "Creating…" : "Open Period"}</Btn></>}>
        <div className="space-y-4">
          <Field label={"Period label"} required><Input value={form.label} onChange={e => setForm(f => ({ ...f, label: e.target.value }))} placeholder={"E.g. Semester 1 — 2026"} /></Field>
          <Field label={"Start date"} required><Input type="date" value={form.date_from} onChange={e => setForm(f => ({ ...f, date_from: e.target.value }))} /></Field>
          <Field label={"End date"} required><Input type="date" value={form.date_to} onChange={e => setForm(f => ({ ...f, date_to: e.target.value }))} /></Field>
        </div>
      </Modal>

      <Modal open={!!editTarget} onClose={() => setEditTarget(null)} title={"Edit Report Period"} size="sm"
        footer={<><Btn variant="ghost" onClick={() => setEditTarget(null)}>{"Cancel"}</Btn><Btn variant="primary" onClick={saveEdit} disabled={savingEdit}>{savingEdit ? "Saving…" : "Save"}</Btn></>}>
        <div className="space-y-4">
          <Field label={"Period label"} required><Input value={editForm.label} onChange={e => setEditForm(f => ({ ...f, label: e.target.value }))} placeholder={"E.g. Semester 1 — 2026"} /></Field>
          <Field label={"Start date"} required><Input type="date" value={editForm.date_from} onChange={e => setEditForm(f => ({ ...f, date_from: e.target.value }))} /></Field>
          <Field label={"End date"} required><Input type="date" value={editForm.date_to} onChange={e => setEditForm(f => ({ ...f, date_to: e.target.value }))} /></Field>
        </div>
      </Modal>
    </div>
  );
