...
export default function StudentFormModal({ hook }: { hook: MemberPrivateDataHook }) {
  ...
  return (
    <Modal
      ...
      title={editTarget ? "Edit Private Student" : "Add Private Student"}
      size="lg"
      footer={
        <>
          <Btn variant="ghost" onClick={() => setOpenForm(false)}>{"Cancel"}</Btn>
          <Btn variant="primary" onClick={saveStudent} disabled={saving}>{saving ? "Saving…" : "Save"}</Btn>
        </>
      }
    >
      <div className="space-y-4">
        {!editTarget && branches && branches.length > 1 && (
          <Field label={"Center"} required hint={"Which center this private student belongs to"}>
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <Field label={"Login email"} required={!editTarget}>
        ...
        {!editTarget && (
          <>
            <SectionLabel sub={"The package price and session count this student is starting with"}>{"Package"}</SectionLabel>
            <div className="grid sm:grid-cols-2 gap-3">
              <Field label={"Package Price (Rp)"} hint={"Total price for the whole package, not per session"}>
                <Input type="number" min={0} value={form.package_price} onChange={e => setForm(f => ({ ...f, package_price: e.target.value }))} />
              </Field>
              <Field label={"Number of Sessions"} hint={"How many sessions this package includes"}>
        ...
        <SectionLabel sub={"Which day(s) and time this student's private lesson happens"}>{"Weekly Schedule"}</SectionLabel>
        ...
        <SectionLabel sub={"Where lessons happen — freely changeable any time"}>{"Location"}</SectionLabel>
        <div className="flex items-center gap-3">
          <Switch
            ...
            label={form.location_type === "external"
              ? "Somewhere else"
              : selectedBranchName
                ? `This center's pool (${selectedBranchName})`
                : "This center's pool"}
          />
        </div>
        {form.location_type === "branch" && (
          <p className="text-xs text-ink-mute -mt-1">{(<>{"Lessons happen at "}<NoTranslate>{selectedBranchName || "—"}</NoTranslate>{"'s own pool — this is the branch this student belongs to."}</>)}</p>
        )}
        {form.location_type === "external" && (
          <div className="space-y-3">
            ...
            <Field label={"Pin the location"} hint={"Used to check the coach's distance when they clock in"}>
              <MapPicker
        ...
        <SectionLabel sub={"A private student can have one head coach plus any number of assistant coaches"}>{"Coach"}</SectionLabel>
        {editTarget && editClassId ? (
          <div>
            <div className="space-y-1.5">
              ...
              {editClassCoaches.length === 0 && <span className="text-xs text-warn-600 font-semibold">{"No coach assigned yet"}</span>}
            </div>
            {(() => {
              ...
              return (
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <Btn variant="soft" size="sm" disabled={!addCoachId || coachMutating} onClick={() => addStudentCoach(editClassId, addCoachId)}>{"Add"}</Btn>
                </div>
              );
            })()}
          </div>
          ...
          <div className="space-y-3">
            <Field label={"Head Coach"} hint={"The main coach responsible for this student"}>
              <Select value={newHeadCoachId} onChange={e => setNewHeadCoachId(e.target.value)}>
                <option value="">{"— not assigned yet —"}</option>
                {coachesForBranch(form.target_branch_id || branchId || null).map(c => <option key={c.id} value={c.id} translate="no">{c.full_name}</option>)}
              </Select>
            </Field>
            <Field label={"Assistant Coach(es)"} hint={"Select any number of assistant coaches"}>
              <div className="space-y-1.5 max-h-36 overflow-y-auto border border-line rounded-xl p-2.5 bg-paper-tint">
                ...
                {coachesForBranch(form.target_branch_id || branchId || null).filter(c => c.id !== newHeadCoachId).length === 0 && (
                  <div className="text-xs text-ink-mute p-1 text-center">{"No other coaches available"}</div>
                )}
              </div>
            </Field>
          </div>
