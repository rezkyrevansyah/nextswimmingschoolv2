...
export default function AwardsTab({ hook }: { hook: AdminCompetitionHook }) {
  const {
    ...
    pickerPaginatedStudents, pickerFilteredStudents, pickerTotalPages, pickerSafePage, setPickerPage,
    setAwardStudentId, setAwardStudentSearch,
  } = hook;
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2.5 flex-wrap">
        ...
        >
          ...
          <option value="reguler">{STUDENT_TYPE_LABELS.reguler}</option>
          <option value="private">{STUDENT_TYPE_LABELS.private}</option>
          <option value="school_affiliate">{STUDENT_TYPE_LABELS.school_affiliate}</option>
        </select>
      ...
      <div className="bg-paper border border-line rounded-2xl overflow-hidden shadow-xs">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-line">
              {pickerPaginatedStudents.map(m => (
                <tr
                  key={m.id}
                  onClick={() => { setAwardStudentId(m.id); setAwardStudentSearch(m.full_name); }}
                ...
                >
                  ...
                  <td className="py-3 px-5 text-ink-mute font-mono text-xs"><NoTranslate>{m.student_no ?? "—"}</NoTranslate></td>
                  <td className="py-3 px-5 text-ink-soft text-sm">{STUDENT_TYPE_LABELS[m.type] ?? m.type}</td>
                  {!branchId && <td className="py-3 px-5 text-ink-soft text-sm"><NoTranslate>{m.branch_name || "—"}</NoTranslate></td>}
                  <td className="py-3 px-5 text-right">
                    <button
                      type="button"
                      onClick={e => { e.stopPropagation(); setAwardStudentId(m.id); setAwardStudentSearch(m.full_name); }}
              ...
              {pickerPaginatedStudents.length === 0 && (
                <tr>
        ...
        {pickerTotalPages > 1 && (
          <div className="flex items-center justify-between gap-2 px-5 py-3 border-t border-line text-xs text-ink-mute bg-paper-tint/30">
            <span>{pickerFilteredStudents.length} student · hal. {pickerSafePage + 1}/{pickerTotalPages}</span>
